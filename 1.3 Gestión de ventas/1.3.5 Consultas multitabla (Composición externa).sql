...
SELECT 'CLIENTE' AS tipo, c.apellido1, c.apellido2, c.nombre FROM cliente c LEFT JOIN pedido 
p ON c.id = p.id_cliente WHERE p.id IS NULL UNION SELECT 'COMERCIAL' AS tipo, co.apellido1, co.apellido2,
co.nombre FROM comercial co LEFT JOIN pedido p ON co.id = p.id_comercial WHERE p.id IS NULL 
